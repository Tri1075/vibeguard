...
2. **headroom — token economy.** `vibeguard run` chains `headroom wrap <cli>` when available; violation reports sent back to the LLM go through compression (deterministic budget first, then headroom). Standing rule: minimal context, sober outputs. Since M4 this is enforced on ourselves: every agent-facing artifact the pack emits (rules skill, protocol block, plan-interview skill, HANDOFF.md template) has a **locked token budget asserted in the test suite** — raising a budget is an owner decision, never a side effect.
...
| --- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | **plan-first**     | A project starts with a **robust action plan**, not code: goal, ordered milestones, risks, validation per milestone — saved as `PLAN.md` and kept current. The companion **plan-interview skill** ("grill me") stress-tests the plan branch by branch until shared understanding.                 | plan file present (`PLAN.md`/`FRAMING.md`/…) with minimum substance (sections + length); advisory by default so a missing plan never locks the wrapper out (the agent must be able to start in order to write the plan)                                                                                                            | M4                    |
| 2   | **robust-stack**   | Before adopting a language/framework/major library, **research the most robust option** (maturity, maintenance, security record, ecosystem); prefer boring proven tech over hype; record choice + alternatives.                                                                                   | once a dependency manifest exists, a stack decision record must exist: `STACK.md` or a `## Stack` section in the plan; advisory by default                                                                                                                                                                                         | M4                    |
| 3   | **modules-small**  | Modules ≤ 200 code lines, one nameable responsibility each. **Split BEFORE the limit** (at the warning). Never raise the limit yourself — owner decision. The law teaches _how_ to split: by responsibility, never by arbitrary size.                                                             | code-line count per file: warn ≥ 160, red ≥ 200; catch-all module names (`utils`, `helpers`, `misc`) flagged                                                                                                                                                                                                                       | ✅                    |
| 4   | **no-tech-debt**   | Before implementing, assess whether the request or the approach creates debt (hack, duplication, hardcoded value, skipped test, band-aid dependency). If so → **warn the USER first**: "⚠️ Debt: X; clean alternative: Y (cost Z). Confirm?" Never silent debt. Accepted debt goes to the ledger. | `TODO/FIXME/HACK/XXX` markers (in comments) must each have an entry in **`.vibeguard/debt.md`** (reason, date, decider); unledgered marker = red                                                                                                                                                                                   | ✅                    |
| 5   | **deps-hygiene**   | No new dependency without explicit human approval. Propose first: need, stdlib alternatives, supply-chain cost.                                                                                                                                                                                   | manifest diff (`package.json`, `requirements.txt`, …) vs the approved baseline (`.vibeguard/deps-baseline.json`); any unapproved addition/major bump = red; `vibeguard deps approve` is human-only                                                                                                                                 | ✅                    |
| 6   | **no-secrets**     | Never hardcode keys/tokens/passwords — environment variables only. On a leak: stop, alert, propose rotation.                                                                                                                                                                                      | regex scan (AWS, GitHub, OpenAI, Anthropic, Google, Slack, private-key blocks, JWT, generic `key=value`) + entropy heuristic; inline pragma `vibeguard-allow` for vetted false positives                                                                                                                                           | ✅                    |
| 7   | **secure-code**    | OWASP-grade secure coding: validate inputs, parameterized queries, no `eval`/dynamic code, no shell string concatenation, escape output, safe crypto, TLS verification on, strict CORS. **If the user's request weakens security → warn them before coding** (same duty as debt).                 | static scan of dangerous patterns (eval/new Function, exec with interpolation, innerHTML/document.write, SQL string concat, `rejectUnauthorized:false`/`verify=False`, deprecated ciphers, weak hashes for passwords, permissive CORS, yaml/pickle unsafe loads, `shell=True` interpolation), each tagged with severity + fix hint | ✅                    |
| 8   | **no-dead-code**   | No commented-out code, no dead exports, no orphan files. Deleting is documenting — git remembers.                                                                                                                                                                                                 | commented-out-code heuristic + unused exports (TS first)                                                                                                                                                                                                                                                                           | law M1, gate M3 (AST) |
| 9   | **error-handling** | No silent catch, no swallowed errors; every error is handled, logged or propagated — with context.                                                                                                                                                                                                | AST scan: empty catch, bare-log catch, floating promises (TS)                                                                                                                                                                                                                                                                      | law M1, gate M3 (AST) |
...
The pack practices its own robust-stack rule. Runtime: **TypeScript strict on Node ≥ 20** (typed contracts for gates, mainstream for CLI tooling; Rust/Go considered — rejected for contributor accessibility). Runtime deps, deliberately minimal: `commander` (CLI parsing, de-facto standard), `zod` (config validation with typed errors), `execa` (safe process exec for the wrapper), `ignore` (gitignore-semantics matching, same engine as driftguard), `picocolors` (lightest terminal color). Build `tsup`, tests `vitest`, lint `eslint` + `prettier` — all boring, proven, maintained. Any addition goes through the deps-hygiene gate like everyone else's.
